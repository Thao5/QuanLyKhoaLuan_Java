...
public class StatsRepositoryImpl implements StatsRepository{
    ...
    public List<Object[]> statTuanSuatTheoNganh(Map<String, String> params) {
        Session s = this.factory.getObject().getCurrentSession();
        Query q = s.createQuery("select id, tenKhoaLuan, sum(nguoiDungSet.size()) from KhoaLuanTotNghiep group by nganh");
        if(params != null){
            String nganh = params.get("nganh");
            if(nganh != null && !nganh.isEmpty()){
                q = s.createQuery("select id, tenKhoaLuan, sum(nguoiDungSet.size()) from KhoaLuanTotNghiep where nganh = :nganh group by nganh");
